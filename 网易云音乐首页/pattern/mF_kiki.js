myFocus.extend({//*********************kiki******************
	mF_kiki:function(par,F){
		...
		var pic=F.$li('pic',box),txt=F.$li('txt',box),btn=F.$li('btn',box),pre=F.$c('prev',box),nex=F.$c('next',box),n=txt.length,start=true;
		...
		eval(F.switchMF(function(){},function(){
			...
			if(!start){F.stop(pic[prev]),pic[prev].style.cssText='left:0;top:0;z-index:3';}
			if(!start){F.stop(pic[next]),pic[next].style.cssText='left:0;top:0;z-index:2';}
			...
			F.slide(pic[next],p_s1,300,'linear',function(){txt[next].style.display='block',this.style.zIndex=3,F.slide(this,p_e,800,'easeOut');}),start=false;
		}));
